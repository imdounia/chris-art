...
class Eh_Stripe_Admin_Handler  {
    ...
    function register_stripe_menu_page() {
        ...
        add_submenu_page( 
            'wt_stripe_menu',
            __( 'Stripe Alipay','payment-gateway-stripe-and-woocommerce-integration' ), 
            __( 'Stripe Alipay','payment-gateway-stripe-and-woocommerce-integration' ),
            'manage_options', 
            'stripe_alipay_menu', 
            array( $this, 'eh_stripe_alipay_menu_page' )
        );
        add_submenu_page( 
            'wt_stripe_menu',
            __( 'Stripe Checkout','payment-gateway-stripe-and-woocommerce-integration' ), 
            __( 'Stripe Checkout','payment-gateway-stripe-and-woocommerce-integration' ),
            'manage_options', 
            'stripe_checkout_menu', 
            array( $this, 'eh_stripe_checkout_menu_page' )
        );
    ...
    function eh_menu_admin_notices(){
        ...
        if(! $notice_dismissed){
            ...
            $eh_stripe = get_option("woocommerce_eh_stripe_pay_settings");
            if ('yes' === $eh_stripe['enabled'] && ('' != $eh_stripe['eh_stripe_test_secret_key'] || '' != $eh_stripe['eh_stripe_live_secret_key'] ) && ('' != $eh_stripe['eh_stripe_test_publishable_key'] || '' != $eh_stripe['eh_stripe_live_publishable_key'] )) {
                return;
            }
            $msg = sprintf('<p style="font-weight: 600;">'. __('Some useful links : ', 'payment-gateway-stripe-and-woocommerce-integration').'</p><table class="eh-admin-notice-table"><tr></tr><tr>
                <td><span>'. __('Stripe is available for businesses in over 30 countries', 'payment-gateway-stripe-and-woocommerce-integration').'</span></td><td> - </td>
        ...
        //admin notice for showing error with api keys
        $page = (isset($_GET['page'])) ? esc_attr($_GET['page']) : false;
        if ('wt_stripe_menu' != $page)
            return;

        $eh_stripe = get_option("woocommerce_eh_stripe_pay_settings");
        if(isset($eh_stripe)){
            if ('test' === $eh_stripe['eh_stripe_mode']) {
            ...
            } elseif('live' == $eh_stripe['eh_stripe_mode']){
    ...
    function eh_stripe_menu_page(){
        ...
        <div class="wrap">
            ...
            <h2><?php _e('WebToffee Stripe','payment-gateway-stripe-and-woocommerce-integration'); ?></h2>
            ...
            ?>
            <div class="eh_settings_left">
                <h2 class="nav-tab-wrapper">
                    <a href="?page=wt_stripe_menu&tab=general" class="nav-tab <?php echo $active_tab == 'general' ? 'eh-nav-tab-active' : ''; ?>"><?php  _e('Stripe','payment-gateway-stripe-and-woocommerce-integration'); ?></a>
                    <a href="?page=wt_stripe_menu&tab=alipay" class="nav-tab <?php echo $active_tab == 'alipay' ? 'eh-nav-tab-active' : ''; ?>"><?php  _e('Stripe Alipay','payment-gateway-stripe-and-woocommerce-integration'); ?></a>
                    <a href="?page=wt_stripe_menu&tab=checkout" class="nav-tab <?php echo $active_tab == 'checkout' ? 'eh-nav-tab-active' : ''; ?>"><?php  _e('Stripe Checkout','payment-gateway-stripe-and-woocommerce-integration'); ?></a>
                    <a href="?page=wt_stripe_menu&tab=help_tab" class="nav-tab <?php echo $active_tab == 'help_tab' ? 'eh-nav-tab-active' : ''; ?>"><?php  _e('Help Guide','payment-gateway-stripe-and-woocommerce-integration'); ?></a>
                    <a class="nav-tab" style="background:#5ccc96; color:#fff;" href="https://www.webtoffee.com/product/woocommerce-stripe-payment-gateway/" target="_blank"><?php _e('Upgrade to Premium for More Features','payment-gateway-stripe-and-woocommerce-integration');?></a>
                            ...
                            ?>
                                ...
                                <form method="post" class="eh_mainform" action="" enctype="multipart/form-data">
                                    ...
                                    wc_enqueue_js("
                                        ...
                                        jQuery( '#woocommerce_eh_stripe_pay_eh_stripe_apple_pay' ).on( 'change', function() {
                                            var apple_fields   = jQuery( '#woocommerce_eh_stripe_pay_eh_stripe_apple_pay_description,#woocommerce_eh_stripe_pay_eh_stripe_apple_pay_position_checkout,#woocommerce_eh_stripe_pay_eh_stripe_apple_pay_on_checkout_page,#woocommerce_eh_stripe_pay_eh_stripe_apple_pay_on_cart_page,#woocommerce_eh_stripe_pay_eh_stripe_apple_pay_on_product_page,#woocommerce_eh_stripe_pay_eh_stripe_apple_pay_type,#woocommerce_eh_stripe_pay_eh_stripe_apple_color,#woocommerce_eh_stripe_pay_eh_stripe_apple_pay_spiliter,#woocommerce_eh_stripe_pay_eh_stripe_apple_pay_language').closest( 'tr' );
                                             ...
                                             <div class="eh_sub_tab_content" data-id="help-links" style="display:block;">
                                                 ...
                                                 <ul class="eh-help-links">
                                                     <li>
                                                         ...
                                                         <p><?php _e('Refer to our documentation to set and get started','payment-gateway-stripe-and-woocommerce-integration'); ?></p>
                                                         <a target="_blank" href="https://www.webtoffee.com/category/documentation/stripe-payment-gateway-for-woocommerce/" class="button button-primary">
                                                     ...
                                                     <li>
                                                         ...
                                                         <h3><?php _e('Help and Support','payment-gateway-stripe-and-woocommerce-integration'); ?></h3>
        ...
        <script type="text/javascript">
            (function (jQuery) {
                ...
                jQuery('.eh_mainform').submit(function(e){
                        
                    var settings_div = jQuery('<div class="eh_msg_div" style="background:#1de026; border:solid 1px #2bcc1c;">Changes saved</div>');				
                    save_settings(settings_div);
                });
                
                var save_settings = function(settings_div)
                {
                    jQuery('body').append(settings_div);
    ...
    /**
	 * Renders alipay settings page
     * @since 3.3.6
	 */
    function eh_stripe_alipay_menu_page(){
        $_GET[ 'tab' ] = 'alipay';
        $this->eh_stripe_menu_page();
       
    }
    
    /**
	 * Renders stripe-checkout settings page
     * @since 3.3.6
	 */
    function eh_stripe_checkout_menu_page(){
        $_GET[ 'tab' ] = 'checkout'; 
        $this->eh_stripe_menu_page();
    }
